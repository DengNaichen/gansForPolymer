...
directions = np.load('../../data/self_avoid/directions_16.npy')
...
for i in range(20):
    ...
    pro_out.save_model(gen, disc, 'random_models/yousef_model', total_epoch)
    loss_value_disc[f'epoch{total_epoch}'] = disc_loss
    loss_value_gen[f'epoch{total_epoch}'] = gen_loss

with open('yousef_gen_loss_three_layers.json', 'w') as fp:
    json.dump(loss_value_gen, fp)
with open('yousef_disc_loss_three_layers.json', 'w') as fp:
