...
for i in range(20):
    ...
    pro_out.save_model(gen, disc, 'random_models/three_layers/model', total_epoch)
    loss_value_disc[f'epoch{total_epoch}'] = disc_loss
    loss_value_gen[f'epoch{total_epoch}'] = gen_loss

with open('random_gen_loss_three_layers.json', 'w') as fp:
    json.dump(loss_value_gen, fp)
with open('random_disc_loss.json_three_layers', 'w') as fp:
